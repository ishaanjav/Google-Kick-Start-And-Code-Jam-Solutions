...
class JudgeSingleCase(object):
    ...
    def _ParseContestantInput(self, response):
        ...
        try:
            ...
            if len(response) != 5:
                # print("YOU: ", response)
                return None, _ERROR_MSG_INCORRECT_WORD_LENGTH + " :== " + response + "," + self.answer
            if ''.join(sorted(response)) != 'ABCDE':
                return None, _ERROR_MSG_INVALID_PERMUTATION + " :== " + response + "," + self.answer
